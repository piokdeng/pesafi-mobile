...
export type BusinessProfile = {
  ...
  business_type: string; // e.g. 'retail', 'restaurant', 'services', 'technology', 'other'
  country: string;
...
export type Invoice = {
  ...
  subtotal: number;
  fee_percent: number; // PesaFi fee e.g. 0.5
  total: number; // USD
  currency: string;
...
export type PayrollBatch = {
  id: string;
  business_id: string;
  label: string; // e.g. "April 2024 Payroll"
...
export type PaymentLink = {
  ...
  business_id: string;
  label: string;
  amount_usd?: number; // null = customer sets amount
  currency: string;
  description?: string;
  url: string;
  is_active: boolean;
...
export type ApiKey = {
  ...
  business_id: string;
  label: string;
  key_preview: string; // e.g. "pk_live_••••••••1234"
